...
| **Autonomía** (cron del Arquitecto, fuera del repo) | job «Guardián de estado — Belleza App» lunes 9:00 + `scripts/guardian-belleza.sh` (perfil Hermes) | que el proyecto se mida solo y quede el parte | **✓ verificado 2026-09-25** ejecutando el runner en su workdir real: inyecta copia inspeccionada, 12 ramas locales con commits fuera de `main`, ramas del remoto, PRs abiertos, `tags archive/*` y `estadoKB.js --check` (`exit=1` por los 2 planes de `.hermes/plans/`). Primer run: **2026-09-28 09:00** (`executions.db` = 0 filas todavía). **Defectos:** el runner siempre sale `0` (aunque el chequeo falle o no exista) y el parte sólo vive en el chat ⇒ **CI-24 / O-014** |
