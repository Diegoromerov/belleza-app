...
- **Antes de toda verificación que lea líneas, declará los finales de línea de tu worktree** (con el número, sobre un archivo de referencia). Tres worktrees del mismo commit pueden no coincidir: banco 1851 CRLF, ejecutor 0, KB 1853 ⇒ una medición en el banco o en la KB no predice lo que verá el CI (LF).
- **Un defecto se arregla por clase, no por sitio.** Tras CI-31 correspondía barrer el resto de las verificaciones que leen líneas: se hizo (CRLF real vs LF) y **sólo el escáner** era ciego ⇒ el barrido se registra aunque haya dado negativo, porque acota el riesgo.
- **Un `Test suite failed to run` por error circular no es un hallazgo del producto: es el arnés.** El worker muere al serializar el error y jest marca a una suite **víctima** al azar ⇒ no la cuentes como roja y declara el `failed-to-run` en toda tabla. El número que se cita es el **mínimo medido con 0 `failed-to-run`**.
- **Un verificador que sale 0 habiendo omitido todo certifica sin medir.** `verifyTenantIsolation.js` imprime «✅ AISLAMIENTO VERIFICADO» con exit 0 tanto con **0 pruebas ejecutadas** (base vacía) como con **22** ⇒ el exit no distingue «pude medir» de «no había nada que medir». Comprobá siempre el **conteo de pruebas ejecutadas**, no sólo el exit.
...
- **Mi entorno de medición tiene que replicar el del CI variable por variable, no aproximarlo.** Faltarme `JWT_SECRET` me hizo publicar 4 tests rojos falsos y acusar al Ejecutor en tres rondas seguidas (`adminPreciosRoutes`). Antes de publicar un número del gate: enumerar las variables del `ci.yml`, exportarlas **todas**, y declarar cuáles se exportaron junto al número.
- **Cuando dos mediciones del mismo commit no coinciden, la hipótesis a descartar primero es la propia** (entorno, finales de línea, base), no la del otro. Pasó dos veces: la ceguera al CRLF (CI-31) y el `JWT_SECRET`.
- **Una suite cuyo veredicto depende de una variable de entorno no mide lo que dice medir.** Es la misma clase que la anterior: dos entornos, dos verdades sobre el mismo commit (CI-41).
