/**
 * R6-RECOVERY-2 Final Retry for Last 5 NULL chunks
 */
...
async function main() {
  ...
  const lastNullChunks = [
    'skincare-neurocosmetica-008',
    'colorimetria-interaccion-farmacos-001',
    'hiperqueratosis-periungueal-008',
    'skincare-osmolaridad-003',
    'skincare-neurocosmetica-005'
  ];

  for (const chunkId of lastNullChunks) {
    ...
    if (dbRes.rows.length > 0) {
      ...
      try {
        ...
        console.error(`${chunkId} ✗ ${err.message}`);
      }
    }
