/**
 * R6-RECOVERY-2 Pre-Rebuild Backup
 * Crea backup del estado actual (todos NULL) antes de reconstruir
 */
...
const BACKUP_PATH = './R6_RECOVERY2_PRE_REBUILD.sql';
...
async function main() {
  ...
  const rows = res.rows;
  const nullCount = rows.filter(r => r.embedding === null).length;

  let sql = '-- R6-RECOVERY-2 PRE-REBUILD BACKUP\n';
  ...
  console.log('PRE-BACKUP: COMPLETE');
