require('dotenv').config({ path: __dirname + '/../.env' });
const youCamClient = require('../src/services/biometric/youcam.client');
const fs = require('fs');
const path = require('path');

async function runClientTest() {
  console.log('=== TEST DEL CLIENTE DE BIOMETRIA CON YOUCAM AI ===');
  
  // Crear una imagen JPEG de prueba más grande (100x100 px, color sólido)
  // Usamos un buffer de imagen JPEG válido mínimo
  const jpegHeader = Buffer.from([
    
  ]);
  
  const base64Image = jpegHeader.toString('base64');
  console.log(`Imagen de prueba: ${base64Image.length} chars base64 (~${Math.round(jpegHeader.length/1024)}KB)`);

  try {
    const result = await youCamClient.analyzeFace(base64Image);
    console.log('\n✅ Diagnóstico facial obtenido exitosamente de YouCam AI:');
    console.log(JSON.stringify(result, null, 2));
    
    // Verificar que los valores NO son los hardcodeados por defecto
    const defaults = { hydration: 75, wrinkles: 15, spots: 12, pores: 25, subtono: 'cálido', bioAge: 28 };
    let allDefault = true;
    for (const [key, defaultVal] of Object.entries(defaults)) {
      if (result[key] !== defaultVal) {
        allDefault = false;
        console.log(`\n✅ ${key}: ${result[key]} (DIFERENTE del default ${defaultVal})`);
      } else {
        console.log(`\n⚠️  ${key}: ${result[key]} (IGUAL al default ${defaultVal})`);
      }
    }
    
    if (allDefault) {
      console.log('\n⚠️  ADVERTENCIA: Todos los valores son los hardcodeados por defecto. La API puede estar devolviendo valores por defecto o el plan no tiene acceso a análisis real.');
    } else {
      console.log('\n✅ CONFIRMADO: Al menos algunos valores provienen de la respuesta real de la API (no son fijos).');
    }
    
  } catch (error) {
    console.error('\n❌ Error:', error.message);
    if (error.response) {
      console.error('Response status:', error.response.status);
      console.error('Response data:', JSON.stringify(error.response.data, null, 2));
    }
  }
}

runClientTest();
