...
class ApiService {
    ...
    for (final port in _ports) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (data['status'] == 'OK' || data['message']?.toString().contains('funcionando') == true) {
            _cachedBaseUrl = url;
            if (kDebugMode) {
  ...
  static dynamic _normalizeDynamicUrls(dynamic value) {
    if (value is Map<String, dynamic>) {
      return value.map((key, entryValue) {
        ...
        return MapEntry(
          key,
          shouldNormalize ? normalizeUrl(entryValue) : _normalizeDynamicUrls(entryValue),
        );
      });
    }
  ...
  static Future<Map<String, dynamic>> fetchProviderDetails(String providerId) async {
    await ensureBaseUrl();
    final response = await http.get(Uri.parse('$_baseUrl$_apiPath/providers/$providerId')).timeout(const Duration(seconds: 30));
    if (response.statusCode == 200) {
      final data = _normalizeDynamicUrls(json.decode(response.body));
  ...
  static Future<Map<String, dynamic>> submitReview(String bookingId, int rating, String comment) async {
    final headers = await _getAuthHeaders();
    final response = await http.post(
      Uri.parse('$_baseUrl$_apiPath/bookings/$bookingId/review'),
      headers: headers,
      body: json.encode({'rating': rating, 'comment': comment.isNotEmpty ? comment : null}),
    ).timeout(const Duration(seconds: 30));
    if (response.statusCode == 200 || response.statusCode == 201) return json.decode(response.body);
    throw Exception(json.decode(response.body)['error'] ?? 'Error ${response.statusCode}');
  }

  static Future<Map<String, dynamic>> payBooking(String bookingId, String paymentMethod) async {
    final headers = await _getAuthHeaders();
    final response = await http.post(
  ...
  static Future<Map<String, dynamic>> getNailTryonJobStatus(String jobId) async {
    final headers = await _getAuthHeaders();
    final response = await http.get(
  ...
  static Future<List<Map<String, dynamic>>> fetchAvailableSlots({
    ...
    await ensureBaseUrl();
    final response = await http.get(
      Uri.parse('$_baseUrl$_apiPath/providers/$providerId/slots?date=$date&service_id=$serviceId'),
    ).timeout(const Duration(seconds: 30));
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
  ...
  static Future<List<Map<String, dynamic>>> fetchChatMessages(String partnerId) async {
    final headers = await _getAuthHeaders();
    final response = await http.get(
  ...
  static Future<Map<String, dynamic>> sendChatMessage(String receiverId, String message, {String? imagePath}) async {
    final headers = await _getAuthHeaders();
    final response = await http.post(
  ...
  static Future<Map<String, dynamic>> markMessagesAsRead(String partnerId) async {
    final headers = await _getAuthHeaders();
    final response = await http.patch(
  ...
  static Future<Map<String, dynamic>> updateBookingStatus(String bookingId, String newStatus) async {
    final headers = await _getAuthHeaders();
    final response = await http.patch(
